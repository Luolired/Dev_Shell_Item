...
if [ "$#" -lt 3 ] || [ "$1" = "-h" ] || [ "$1" = "--help" ]; then
	...
	echo 
	_content "版权所有,禁止盗版:Create Time:2016-05-04,Author:lizx 007ka-soc,V7.4"
...
function g_fn_Director()
{
    ...
    then
	    ...
	    #回退脚本，这里要找到之前软链接的源文件
            ...
            _info "目标路径===存在同名文件:$file_line 且已有软链接===整理完毕\n\t旧版本: $(cd ${g_p_Dest_Path} && md5sum ${oldlink_file}) \n\t当前版本:$(cd ${g_p_Dest_Path} && md5sum ${dest_line});"
	    ...
	    #开始软链接到新版本
	    [ $? -eq 0 ] && cd $g_p_Dest_Path && sudo -u $sudo_user ln -sf $(echo $dest_line|awk -F '/' '{print $NF}') $file_line
            _info "目标路径===存在同名文件:$file_line 无软链接规范===整理完毕\n\t旧版本重命名为:$(cd $g_p_Dest_Path && md5sum ${source_line});\n\t当前版本:$(cd $g_p_Dest_Path && md5sum ${dest_line});"
        fi
	...
	cd $source_path && sudo -u $sudo_user cp $file_line ${g_p_Dest_Path}/${dest_line}
        #开始软链接到新版本
        [ $? -eq 0 ] && cd $g_p_Dest_Path && sudo -u $sudo_user ln -sf $(echo $dest_line|awk -F '/' '{print $NF}') $file_line
        _info "=== 目标路径无同名文件:$file_line文件首次复制&软链接完毕===\n\t 当前版本:$(cd $g_p_Dest_Path && md5sum ${dest_line})"
	_info "=== 首次复制不产生回滚脚本 ===;"
    fi
}
...
function main()
{
	_info "Now Let us Start Work Job:\n\t本次上线OA流水号:${g_i_OANUM} 将要上线的程序包目录路径或文件:${g_p_ProGram_Path} 生产目标路径:${g_p_Dest_Path}"
