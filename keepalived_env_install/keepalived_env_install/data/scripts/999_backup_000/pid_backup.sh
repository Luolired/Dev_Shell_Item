...
#输出keepalived主备状态
/bin/echo $(date +%c) backup >> /etc/keepalived/state.txt
...
g_s_LOGDATE=`date +"%F"`
#执行脚本生成的日志all
g_s_LOGFILE="${g_s_LOG_PATH}/pid_backup.${g_s_LOGDATE}.log"
...
Stop_Local_Prog()
{
        g_fn_LOG "$G_LOCAL_IP 节点降级为backup调用的脚本 一键关闭本机程序 Start"
        #提前加锁不同步
	touch $RSYNC_PID_LOCK_FILE &> /dev/null
        ...
        g_fn_LOG "$G_LOCAL_IP 节点降级为backup调用的脚本 一键关闭本机程序 End"
}
                ...
                exit 1
                 ...
                 do
                         ...
                         if [ ! -e $RUN_FILE ];then
                                 ...
                                 $RUN_FILE stop
                                 if [ $? -eq 0 ];then
                                         g_fn_LOG "$RUN_FILE stop 已经多开程序关闭"
                                 else
                                         g_fn_LOG "$RUN_FILE stop 本机未开对端程序"
                                 fi
                         fi
                        echo -e '\n'
                 done
...
main(){
		...
		#notify_backup "/etc/keepalived/scripts/pid_backup.sh"
